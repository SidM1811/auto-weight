...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def calculate_shaping_reward(self, prev_state, state, action, shaping_weights=None):
        ...
        if isinstance(shaping_weights, torch.Tensor) or isinstance(shaping_weights, np.ndarray):
            shaping_weights = {
                "distance": shaping_weights[0],
                "velocity": shaping_weights[1],
                "angle": shaping_weights[2],
                "leg_contact": shaping_weights[3],
                "main_engine": shaping_weights[4],
                "side_engine": shaping_weights[5],
            }

        shaping = (
            -shaping_weights["distance"] * np.sqrt(state[0] * state[0] + state[1] * state[1])
            - shaping_weights["velocity"] * np.sqrt(state[2] * state[2] + state[3] * state[3])
            - shaping_weights["angle"] * abs(state[4])
            + shaping_weights["leg_contact"] * state[6]
            + shaping_weights["leg_contact"] * state[7]
        )
        if prev_state is not None:
            prev_shaping = (
                -shaping_weights["distance"] * np.sqrt(prev_state[0] * prev_state[0] + prev_state[1] * prev_state[1])
                - shaping_weights["velocity"] * np.sqrt(prev_state[2] * prev_state[2] + prev_state[3] * prev_state[3])
                - shaping_weights["angle"] * abs(prev_state[4])
                + shaping_weights["leg_contact"] * prev_state[6]
                + shaping_weights["leg_contact"] * prev_state[7]
            )
            reward = shaping - prev_shaping
        else:
            reward = 0

        m_power = action[0] if isinstance(action, np.ndarray) else (1.0 if action == 2 else 0.0)
        s_power = abs(action[1]) if isinstance(action, np.ndarray) else (1.0 if action in [1, 3] else 0.0)

        reward -= m_power * shaping_weights["main_engine"]
        reward -= s_power * shaping_weights["side_engine"]

        return reward

    def calculate_next_state(self, action, current_state=None):
        if current_state is not None:
            # Save the current environment state
            env_state = self.lander.position, self.lander.linearVelocity, self.lander.angle, self.lander.angularVelocity
            # Set the lander to the provided current state
            self.lander.position, self.lander.linearVelocity, self.lander.angle, self.lander.angularVelocity = current_state
        # Apply the action to the lander
        self.step(action)
        # Get the next state
        next_state = self.lander.position, self.lander.linearVelocity, self.lander.angle, self.lander.angularVelocity
        if current_state is not None:
            # Restore the environment state
            self.lander.position, self.lander.linearVelocity, self.lander.angle, self.lander.angularVelocity = env_state
        return next_state
    
    def readout_internal_state(self):
        return self.lander.position, self.lander.linearVelocity, self.lander.angle, self.lander.angularVelocity
    
    def readout_external_state(self):
        pos = self.lander.position
        vel = self.lander.linearVelocity
        state = [
            (pos.x - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2),
    ...
    def set_internal_state(self, state):
        self.lander.position, self.lander.linearVelocity, self.lander.angle, self.lander.angularVelocity = state
