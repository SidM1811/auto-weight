...
if __name__ == "__main__":
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env_id(args.env_id, i, args.capture_video, run_name) for i in range(args.num_envs)],
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            shaped_rewards = torch.tensor([
                env.unwrapped.calculate_shaping_reward(
                    curr_obs[i],
                    next_obs[i],
                    action[i],
            ...
            # To handle shaping after termination / truncation
            rewards[step] += torch.where(next_done > 0.0, torch.tensor(0.0).to(device), shaped_rewards)
